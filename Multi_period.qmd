...
# Flatten all A matrices into a DataFrame
coeffs = []
for t in range(n_time_steps):
    for sample_idx, A in enumerate(A_all[t]):
        for i in range(n_agents):
            for j in range(n_agents):
                coeffs.append({
                    ...
                    'Row': i,
                    'Col': j,
                    'Coefficient Value': A[i, j],
                    'Index': f'a{i+1}{j+1}'
                })

df_coeffs = pd.DataFrame(coeffs)

# Plot: Boxplot of coefficients by time step and position (a_ij)
plt.figure(figsize=(12, 6))
sns.boxplot(data=df_coeffs, x='Time Step', y='Coefficient Value', hue='Index')
plt.title('Boxplot of Coefficients for All A_t Matrices (A0 to A6)')
plt.grid(True)
plt.tight_layout()
plt.legend(title='Matrix Entry')
plt.show()
